class Film < ApplicationRecord
  ...
  def self.get_random_film(genre = nil, year = nil)
    ...
    if film_results.present? && movie_valid?(film_results)
  ...
  def self.find_or_create_film(film_json)
    film = Film.find_by(mdb_id: film_json["id"]) || Film.create(
      mdb_id: film_json["id"],
  ...
  def self.parse_providers(providers)
    return "Not available to stream or rent in your region." if providers["streaming_providers"].nil? || providers["rental_providers"].nil?
    parse_array = []
  ...
  def self.get_random_film_args(param_array)
    hash = {}
    hash["year"] = param_array.include?("year") ? param_array[param_array.index("year") + 1] : nil
    hash["genre"] = param_array.include?("genre") ? param_array[param_array.index("genre") + 1] : nil
    hash
  end
